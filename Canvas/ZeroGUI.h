...
namespace ZeroGUI
{
	namespace Colors
	{
		FLinearColor Text{ 255.0f, 255.0f, 255.0f, 1.0f };
		FLinearColor Text_Shadow{ 255.0f, 255.0f, 255.0f, 1.0f };
		...
		FLinearColor Button_Active{ 255.0f, 0.0f, 0.0f, 1.0f };

		FLinearColor Checkbox_Idle{ 255.0f, 0.0f, 0.0f, 1.0f };
		FLinearColor Checkbox_Hovered{ 255.0f, 0.0f, 0.0f, 1.0f };
		FLinearColor Checkbox_Enabled{ 0.0f, 255.0f, 72.0f, 1.0f };
	...
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		canvas->K2_DrawText(ue->Font, FString{ s2wc(name), }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(ue->Font, FString{ s2wc(name), }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void DrawFilledCircle(FVector2D pos, float r, FLinearColor color)
	{
		...
		double P_I = 3.14159265359;
		int size = (int)(2.0f * P_I / smooth) + 1;

		float angle = 0.0f;
		int i = 0;

		for (; angle < 2 * P_I; angle += smooth, i++)
		{
	...
	{
		float P_I = 3.1415927f;

		float Step = P_I * 2.0 / numSides;
		int Count = 0;
		FVector2D V[128];
		for (float a = 0; a < P_I * 2.0; a += Step) {
	...
	void Hotkey(const char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			//Text
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter(crypt_str("[Press Key]"), textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
